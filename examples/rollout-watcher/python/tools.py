...
def get_signals(args: dict[str, object]) -> dict[str, object]:
    subject = _read_string(args.get("subject")) or "rollout-watcher"
    signal = SIGNALS.get(subject, SIGNALS["rollout-watcher"])
...
def build_rollout_watcher_prompt(task: str) -> str:
    return "\n".join([
        ...
        f"Incident or subject: {task or 'rollout-watcher investigation'}",
    ])
